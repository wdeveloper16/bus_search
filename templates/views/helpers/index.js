...
var linkTemplate = _.template('<a <% if(typeof url !== "undefined"){ %>href="<%= url %>"<% } %>><%= text %></a>');
...
module.exports = function () {
	...
	* */
	_helpers.needsPagination = function (totalPages, options) {
	  if (totalPages == 1) {
	    return options.inverse(this);
	  }
	  return options.fn(this);
	...
	_helpers.paginationNavigation = function (pages, currentPage, totalPages, options) {
		...
		_.each(pages, function (page, ctr) {
			...
			// wrapup the html
			if (isActivePage)
  			html += '<li' + liClass + '>' + linkTemplate({ text: pageText }) + '</li>\n';
			else
	...
	_helpers.escapedouble = function(variable) {
		return variable.replace(/(")/g, '\\$1');
	};
	
	_helpers.prettifyUrl = function (url) {
		return getSlug(url);
	};
	
	_helpers.if_even = function(conditional, options) {
  if((conditional % 2) == 0) {
    return options.fn(this);
  } else {
    return options.inverse(this);
